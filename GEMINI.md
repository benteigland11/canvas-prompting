...
### Domains
    ...
    devops     Infrastructure-as-code, deployment, and operational tooling
...
**Build widgets**
    ...
    create <widget_id> [--language angular|javascript|nim|openscad|php|python|systemverilog|terraform|typescript] [--domain backend|data|devops|frontend|infra|ml|modeling|rtl|security|universal] [--name NAME] [--target .]
      Scaffold a new widget

    rename <widget_id> [--name NAME] [--domain backend|data|devops|frontend|infra|ml|modeling|rtl|security|universal] [--target .]
      Rename a scaffolded widget's slug or domain (pre-checkin, Python-only)

    validate [path] [--lib]
      Run the validation pipeline on a widget

    checkin [path] [--reason REASON] [--bump minor] [--publish] [--no-publish] [--override-warnings] [--override-reason OVERRIDE_REASON]
    ...
    blueprint create <name> [--language angular|javascript|nim|openscad|php|python|systemverilog|terraform|typescript] [--target .]
      Scaffold a new blueprint (a higher-order widget composing other widgets)

    blueprint add-dep <widget_id> [--path .] [--no-validate]
      Add an installed widget to a blueprint's dependencies, pinned to its installed version

    blueprint remove-dep <widget_id> [--path .] [--no-validate]
      Remove a widget from a blueprint's dependencies

    architect init [--path PATH] [--force]
      Scaffold a starter architect.py describing this project's structure

    architect validate [--path PATH]
      Check architect.py for structural issues (refs, ids, parent cycles, domains)

    architect render [--path PATH] [--output OUTPUT] [--stdout] [--direction TD] [--force]
      Render architect.py as a Mermaid flowchart

    architect link <component_id> [widget] [--clear] [--path PATH]
      Link or unlink an installed widget to a Component slot
...
**Cloud registry**
    ...
    cloud publish [widget_id] [path] [--lib] [--visibility public|private] [--governance open|protected] [--override-warnings] [--override-reason OVERRIDE_REASON]
      Publish a widget or blueprint to the cloud registry
